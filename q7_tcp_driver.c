...
#include <arpa/inet.h>
/* FIX 4: removed #include <dirent.h> - no longer needed here */
...
void send_data(PacketType type, uint32_t payload_length, uint8_t *data)
{
    ...
    uint16_t frag_total = (payload_length == 0) ? 1 : (uint16_t)((payload_length + MAX_PAYLOAD_SIZE - 1) / MAX_PAYLOAD_SIZE);

    for (uint16_t frag_id = 0; frag_id < frag_total; frag_id++)
    {
        uint32_t offset = (uint32_t)frag_id * MAX_PAYLOAD_SIZE;
        uint16_t chunk_size = (payload_length - offset < MAX_PAYLOAD_SIZE) ? (uint16_t)(payload_length - offset) : MAX_PAYLOAD_SIZE;
...
/* FIX 4 (cont): send_file_list, send_file, send_latest_image moved to
   q7_tcp_test.c where send_image() is defined and accessible */